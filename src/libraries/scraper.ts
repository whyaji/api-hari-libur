...
export const crawler = async (year: string) => {
  ...
  return Array.from(months).flatMap((item: any) => {
    const [monthName, year] =
      item
        .querySelector(".kal-title .kal-title-link")
        ?.textContent?.split(" ") || [];

    const month =
      MONTH_NAME[monthName.toLocaleLowerCase() as keyof typeof MONTH_NAME];

    return Array.from(item.querySelectorAll(".kal-libur-list li"))
      .flatMap((holiday: any) => {
